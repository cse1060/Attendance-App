...
import "../css/take_attendance.css"
        ...
        axios({
            ...
            .then((data) => {
                ...
                if (data.data.success === true) {
                    ...
                    // arr.push(data.data)
                    // console.log(arr)
                    setReturnData([...returnData, data.data])
    ...
    if (mode === 'post') {
        return (
            <>
                ...
                />
                <div className='take_attendance'>
                    <label htmlFor="image" className='label'>Upload Image</label>
                    <input accept="image/*" type="file" name="image" id="image_upload" files={selectedFile}
                        onChange={(e) => {
                            setSelectedFile(e.target.files);
                            setImage(URL.createObjectURL(e.target.files[0]))
                        }} />
                    {image !== null && <img alt="preview image" className="attendance_img" src={image} />}
                    <br></br>
                    {image !== null && <button onClick={submitForm} >Submit</button>}
        ...
        return (
            <>
                ...
                <div className='more_Img'>
                    <div className='take_attendance2'>
                        <label htmlFor="image" className='label'>Upload More Images / Different Angles</label>
                        <input accept="image/*" type="file" name="image" id="image_upload" files={selectedFile}
                            onChange={(e) => {
                                setSelectedFile(e.target.files);
                                setImage(URL.createObjectURL(e.target.files[0]))
                ...
                <div className='data'>
                    {returnData.map((obj, idx) => {
                        return (
                            <>
                                <CSVLink className='download' data={obj.attendance}>Download CSV</CSVLink>
                                <br></br>
                                <img className="img_checked" src={`data:image/jpeg;base64,${obj.image}`} />
                                <hr className='line-att' />
                                <div className='contain'>
                                    <table class="table">
                                        <thead>
                                            <tr>

                                                <th className="table-secondary" scope="col">S. no.</th>
                                                <th className="table-secondary" scope="col">Roll No.</th>
                                                <th className="table-secondary" scope="col">Attendance</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {obj.attendance.map((att, id) => {
                                                return (
                                                    <tr>
                                                        <th className='table-dark' scope="row">{id}</th>
                                                        <td className='table-dark'>{att[0]}</td>
                                                        <td className='table-dark'>{att[1]}</td>
                                                    </tr>
                                                )
                                            })}
                                        </tbody>
